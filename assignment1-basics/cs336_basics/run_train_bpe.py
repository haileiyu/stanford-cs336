...
def load_word_tuple_to_count(input_path: str | os.PathLike, special_tokens: list[str]) -> dict[tuple[bytes, ...], int]:
    ...
    with open(input_path, "rb") as f:
        ...
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            ...
            wt_to_count = load_word_tuple_to_count_from_chunk(content, special_tokens)
            merge_map(word_tuple_to_count, wt_to_count)

    return word_tuple_to_count


def merge_map(dst: dict[tuple[bytes, ...], int], src: dict[tuple[bytes, ...], int]) -> dict[tuple[bytes, ...], int]:
...
def load_word_tuple_to_count_from_chunk(content: str, special_tokens: list[str]) -> dict[tuple[bytes, ...], int]:
